...
async function main() {
  ...
  console.log("Fetching Last.fm monthly data...");

  const [scrobbles, topArtists, topAlbums, topTracks] = await Promise.all([
    fetchMonthlyScrobbleCounts(),
    fetchTopArtists(10, "1month"),
    fetchTopAlbums(10, "1month"),
    fetchTopTracks(10, "1month"),
  ]);
  ...
  console.log(`${scrobbles.thisMonth} scrobbles this month (${scrobbles.lastMonth} last month), ${topArtists.length} top artists.`);
